...
export default function Register() {
  ...
  const handleTraderSubmit = async (e: React.FormEvent) => {
    ...
    toast({ title: 'Account created!', description: 'Welcome to CargoSpace. You can now start booking.' });
  ...
  const handleProviderSubmit = async (e: React.FormEvent) => {
    ...
    if (providerForm.transportModes.length === 0) {
      toast({ title: 'Select transport modes', description: 'Please select at least one transport mode.', variant: 'destructive' });
      return;
    }
    setIsLoading(true);
    await new Promise(resolve => setTimeout(resolve, 1500));
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          ...
          <p className="text-muted-foreground mb-8">
            Join thousands of traders and providers on our platform
          </p>
              ...
              <form onSubmit={handleTraderSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    value={traderForm.companyName}
                    onChange={e => setTraderForm({ ...traderForm, companyName: e.target.value })}
                  />
                </div>
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    id="trader-terms"
                    checked={traderForm.agreeTerms}
                    onCheckedChange={(checked) => setTraderForm({ ...traderForm, agreeTerms: checked as boolean })}
                  />
              ...
              <form onSubmit={handleProviderSubmit} className="space-y-4">
                <div className="p-4 bg-ocean-light rounded-lg mb-4">
                  <p className="text-sm text-foreground">
                    <strong>Provider Registration:</strong> Your application will be reviewed by our team. Approval typically takes 2-3 business days.
                  </p>
                </div>
                    ...
                    {transportModes.map(mode => (
                      <button
                        ...
                        className={`flex items-center gap-2 p-3 rounded-lg border-2 transition-all ${
                          providerForm.transportModes.includes(mode.id)
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    id="provider-terms"
                    checked={providerForm.agreeTerms}
                    onCheckedChange={(checked) => setProviderForm({ ...providerForm, agreeTerms: checked as boolean })}
                  />
