...
import { Menu, Ship, Package, MessageSquare, User, LogOut, LayoutDashboard } from 'lucide-react';
...
export function Header() {
  ...
  const navLinks = isLoggedIn
    ? [
        ...
        ...(role === 'provider' ? [{ href: '/dashboard', label: 'Dashboard', icon: LayoutDashboard }] : []),
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <div className="hidden lg:flex items-center gap-4">
            {isLoggedIn ? (
              <Button variant={isLandingPage ? 'heroOutline' : 'outline'} onClick={handleLogout}>
                <LogOut className="w-4 h-4 mr-2" />Logout
              </Button>
            ) : (
              <>
                <Link to="/login">
                  <Button variant={isLandingPage ? 'heroOutline' : 'ghost'}>Sign In</Button>
                </Link>
                <Link to="/register">
                  <Button variant={isLandingPage ? 'hero' : 'default'}>Get Started</Button>
                </Link>
              </>
            )}
          </div>
